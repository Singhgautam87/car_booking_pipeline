...
def get_booking_data():
    ...
    if summary and daily:
        return {
            ...
            "payments": payments,
            "loyalty":  loyalty,
            "top_cars": api_get("/api/bookings/routes?limit=7"),
...
def get_monitoring_data():
    try:
        ...
        if sla_api:
            val_df    = pd.DataFrame(val_api)    if val_api    else pd.DataFrame()
            exp_df    = pd.DataFrame(exp_api)    if exp_api    else pd.DataFrame()
            fail_df   = pd.DataFrame(fail_api)   if fail_api   else pd.DataFrame()
            sla_df    = pd.DataFrame(sla_api)
            alerts_df = pd.DataFrame(alerts_api) if alerts_api else pd.DataFrame()
...
def L(**kwargs):
    """Layout helper — builds clean dict from CBASE + kwargs"""
    return {**CBASE, **kwargs}
...
def empty(msg="no data"):
    ...
    fig.update_layout(**L(xaxis=AXIS, yaxis=AXIS, height=220))
    return fig
...
def update_filters(booking_data):
    ...
    kpi_list    = booking_data.get("kpi", [{}])
    kpi_dict    = kpi_list[0] if kpi_list and isinstance(kpi_list, list) else {}
    ...
    loy_opts = [{"label":v,"value":v} for v in sorted(loyalty_df["loyalty_tier"].dropna().unique())] \
                if not loyalty_df.empty and "loyalty_tier" in loyalty_df.columns else []
    pay_opts = [{"label":v,"value":v} for v in sorted(payments_df["payment_method"].dropna().unique())] \
                if not payments_df.empty and "payment_method" in payments_df.columns else []
    seg_opts = [{"label":v,"value":v} for v in sorted(segments_df["car_segment"].dropna().unique())] \
                if not segments_df.empty and "car_segment" in segments_df.columns else []
    ...
    meta_lines = [
        f"records: {kpi_dict.get('total_bookings', 0):,}",
        f"customers: {kpi_dict.get('total_customers', 0):,}",
        f"routes: {kpi_dict.get('unique_routes', 0):,}",
        f"segments: {kpi_dict.get('car_segments', 0):,}",
        f"models: {kpi_dict.get('car_models', 0):,}",
    ]
...
def render_overview(bk, sla, val, alerts, loy_f, pay_f, trip_f, seg_f, clr):
    ...
    kpi_data   = bk.get("kpi", [{}])
    kpi_dict   = kpi_data[0] if kpi_data and isinstance(kpi_data, list) else (kpi_data if isinstance(kpi_data, dict) else {})
    ...
    total_b    = int(kpi_dict.get("total_bookings", 0))
    total_c    = int(kpi_dict.get("total_customers", 0))
    total_rev  = float(kpi_dict.get("total_revenue", 0))
    high_value = int(kpi_dict.get("high_value_customers", 0))
    digital    = int(kpi_dict.get("digital_payments", 0))
    ...
    if not seg_df.empty and "car_segment" in seg_df.columns:
        ...
        fig_seg.update_layout(**L(height=260))
    else:
        fig_seg = empty("car segments"); fig_seg.update_layout(height=260)

    if not cars_df.empty:
        if "route" in cars_df.columns:
            x_col, y_col, label = "bookings", "route", "top routes · bookings"
        elif "model" in cars_df.columns:
            x_col, y_col, label = "revenue", "model", "top cars · revenue"
        else:
            x_col, y_col, label = cars_df.columns[1], cars_df.columns[0], "top items"
        fig_cars = go.Figure(go.Bar(
            x=cars_df[x_col], y=cars_df[y_col], orientation="h",
            marker=dict(color=cars_df[x_col],
                        colorscale=[[0,"#0f2027"],[0.5,C1],[1,C4]], showscale=False),
            text=[f"{v:,.0f}" for v in cars_df[x_col]],
            textposition="outside", textfont=dict(color=SUB,size=9,family=MONO)))
        fig_cars.update_layout(**L(title=label, height=260,
                                   xaxis=AXIS, yaxis={"categoryorder":"total ascending"}))
    else:
        fig_cars = empty("top routes"); fig_cars.update_layout(height=260)
    ...
    if not heat_df.empty and "month" in heat_df.columns:
        ...
        fig_heat.update_layout(**L(xaxis=AXIS, yaxis=AXIS, coloraxis_showscale=False, height=250))
    else:
...
def render_analytics(bk, loy_f, pay_f, trip_f, seg_f, clr):
    ...
    if "booking_date" in df.columns:
        ...
        fig_d.update_layout(**L(title="daily · bookings + revenue", xaxis=AXIS,
    ...
    if "amount_tier" in df.columns:
        ...
        fig_at.update_layout(**L(xaxis=AXIS, yaxis=AXIS, coloraxis_showscale=False, height=260))
    else:
    ...
    if "model" in df.columns:
        ...
        fig_sc.update_layout(**L(xaxis=AXIS, yaxis=AXIS, coloraxis_showscale=False, height=280))
    else:
    ...
    if "insurance_coverage" in df.columns:
        ...
        fig_ins.update_layout(**L(xaxis=AXIS, yaxis=AXIS, height=260))
    else:
    ...
    if "loyalty_tier" in df.columns and "payment_method" in df.columns:
        ...
        fig_lp.update_layout(**L(xaxis=AXIS, yaxis=AXIS, height=270))
    else:
    ...
    if "price_per_day" in df.columns:
        ...
        fig_pr.update_layout(**L(xaxis=AXIS, yaxis=AXIS, height=250))
    else:
...
def render_routes(bk, loy_f, pay_f, trip_f, seg_f, clr):
    ...
    fbadge = _filter_badge(df, df_raw)
    if df.empty:
        return card([html.Div("[ route data not available ]",
    ...
    fig_routes.update_layout(**L(title="top 10 routes · booking volume", height=340,
                              xaxis=AXIS, yaxis={"categoryorder":"total ascending"}))
    ...
    if "pickup_hour" in df.columns and "booking_date" in df.columns:
        ...
        fig_ph.update_layout(**L(xaxis=AXIS, yaxis=AXIS, coloraxis_showscale=False, height=300))
    else:
    ...
    if "pickup_slot" in df.columns:
        ...
        fig_slot.update_layout(**L(title="pickup slot · volume + avg revenue",
                                xaxis=AXIS,
...
def render_observability(bk, loy_f, pay_f, trip_f, seg_f, clr):
    ...
    if "booking_date" in df.columns:
        ...
        fig_vol.update_layout(**L(title=f"volume monitor · {len(anom)} anomalies",
    ...
    fig_null.update_layout(**L(title="null health · key columns",
                            xaxis=AXIS, yaxis=AXIS,
    ...
    if "pickup_hour" in df.columns:
        ...
        fig_hour.update_layout(**L(xaxis=AXIS, yaxis=AXIS, showlegend=False, height=250))
    else:
    ...
    if "booking_date" in df.columns:
        ...
        fig_fr.update_layout(**L(xaxis=AXIS, yaxis=AXIS, coloraxis_showscale=False, height=240))
    else:
...
# ================================================================
# CB 8 — PIPELINE
# ✅ FIX 4: Safe run_id access with try/except
# ================================================================
@app.callback(
...
def render_pipeline(sla, alerts, schema):
    ...
    try:
        runs = sla_df["run_id"].nunique() if "run_id" in sla_df.columns else len(sla_df)
    except Exception:
        runs = len(sla_df)

    avg_d = sla_df["duration_seconds"].mean() if "duration_seconds" in sla_df.columns else 0
    recs  = sla_df["records_processed"].sum() if "records_processed" in sla_df.columns else 0
    s_rate= len(sla_df[sla_df["status"]=="SUCCESS"])/len(sla_df)*100 if "status" in sla_df.columns else 0

    sa = sla_df.groupby("stage_name")["duration_seconds"].agg(["mean","max"]).reset_index() \
         if "stage_name" in sla_df.columns and "duration_seconds" in sla_df.columns \
         else pd.DataFrame(columns=["stage_name","mean","max"])
    if not sa.empty:
        ...
        fig_dur.update_layout(**L(title="stage duration · avg vs max (s)", barmode="group",
                              xaxis=AXIS, yaxis=AXIS,
                              legend=dict(orientation="h",y=1.12,x=0,font=dict(size=10)), height=280))
    else:
        fig_dur = empty("stage duration"); fig_dur.update_layout(height=280)

    sc = sla_df["status"].value_counts().reset_index() if "status" in sla_df.columns else pd.DataFrame()
    if not sc.empty:
        ...
        fig_st.update_layout(**L(height=280))
    else:
        fig_st = empty("stage status"); fig_st.update_layout(height=280)

    # ✅ FIX 4: Safe latest_run access
    try:
        latest_run = sla_df["run_id"].iloc[0] if "run_id" in sla_df.columns and not sla_df.empty else "latest"
        ltd = sla_df[sla_df["run_id"]==latest_run].copy() if "run_id" in sla_df.columns else sla_df.head(10).copy()
    except Exception:
        latest_run = "latest"
        ltd = sla_df.head(10).copy()

    if not ltd.empty:
        ltd["stage_name"] = ltd["stage_name"].str.replace("_"," ") if "stage_name" in ltd.columns else ltd.get("stage_name","")
    ...
    return loading(html.Div([
        ...
        html.Div(style={"marginTop":"12px"}, children=[
            card(accent=C1, mb="0", children=[
                ...
                dash_table.DataTable(data=ltd[show].to_dict("records") if show else [],
...
# ================================================================
# CB 9 — DATA QUALITY
# ✅ FIX 3: Named aggregation syntax fixed for pandas groupby
# ================================================================
@app.callback(
...
def render_quality(val, exp, fail):
    ...
    fig_gauge.update_layout(**L(height=260))

    # ✅ FIX 3: Named aggregation — correct pandas syntax
    fig_rules = empty("validation rules")
    fig_rules.update_layout(height=300)

    if not exp_df.empty and "success" in exp_df.columns:
        try:
            # ✅ FIX 3: Use df.groupby().agg((col, func)) named aggregation — not series.agg(name=func)
            exp_df2 = exp_df.copy()
            exp_df2["success_bool"] = exp_df2["success"].astype(bool)
            es = exp_df2.groupby("expectation_type").agg(
                passed=("success_bool", "sum"),
                failed=("success_bool", lambda x: (~x).sum())
            ).reset_index()
            es["passed"] = es["passed"].astype(int)
            es["failed"] = es["failed"].astype(int)
            ...
            fig_rules.update_layout(**L(xaxis=AXIS, yaxis={"categoryorder":"total ascending"}, height=300))
        except Exception as e:
            print(f"[DQ RULES ERROR] {e}")
            fig_rules = empty(f"validation rules — {str(e)[:40]}")
            fig_rules.update_layout(height=300)

    elif not exp_df.empty and "failure_count" in exp_df.columns:
        try:
            es = exp_df.copy()
            es["passed"] = 0
            es["failed"] = es["failure_count"]
            es["rule"] = (es["expectation_type"]
                          .str.replace("expect_column_values_to_be_","")
                          .str.replace("expect_column_","").str.replace("_"," "))
            em = es.melt(id_vars=["expectation_type","rule"],
    ...
    if "created_at" in val_df.columns:
        ...
        fig_hist.update_layout(**L(xaxis=AXIS, yaxis=AXIS, yaxis_range=[0,105], height=260))
    else:
        fig_hist = empty("quality history"); fig_hist.update_layout(height=260)

    # ✅ FIX 3: Safe column access for passed/failed counts
    vp = 0; vv = 0
    if not exp_df.empty and "success" in exp_df.columns:
        try:
            vp = int(exp_df["success"].astype(bool).sum())
            vv = int((~exp_df["success"].astype(bool)).sum())
        except Exception:
            pass
...
# ================================================================
# CB 10 — AI · ML TAB
# ✅ FIX 1: kpi variable renamed to kpi_ctx — no longer shadows kpi() function
# ✅ FIX 2: All update_layout use L() helper — no duplicate paper_bgcolor
# ================================================================
@app.callback(
...
def render_ai(forecast_data, fraud_data, churn_data, bk):
    ...
    # ✅ FIX 1: Renamed kpi → kpi_ctx to avoid shadowing kpi() function
    kpi_data_ai = bk.get("kpi", [{}]) if isinstance(bk, dict) else [{}]
    kpi_ctx     = kpi_data_ai[0] if kpi_data_ai else {}
    ...
    if not forecast_df.empty and "forecast_date" in forecast_df.columns:
        ...
        # ✅ FIX 2: L() helper — no duplicate paper_bgcolor
        fig_fc.update_layout(**L(title="demand forecast · 30 days (Prophet)",
                              xaxis=AXIS, yaxis=AXIS,
                              legend=dict(orientation="h",y=1.12,x=0,font=dict(size=10)), height=300))
    else:
    ...
    if not fraud_df.empty and "risk_label" in fraud_df.columns:
        ...
        # ✅ FIX 2: L() helper
        fig_fraud_dist.update_layout(**L(height=280))
    else:
    ...
    if not fraud_df.empty and "pickup_hour" in fraud_df.columns and "risk_label" in fraud_df.columns:
        ...
        # ✅ FIX 2: L() helper
        fig_fraud_heat.update_layout(**L(xaxis=AXIS, yaxis=AXIS, coloraxis_showscale=False, height=250))
    else:
    ...
    if not churn_df.empty and "churn_risk" in churn_df.columns:
        ...
        # ✅ FIX 2: L() helper — this was the crash line!
        fig_churn.update_layout(**L(title="churn risk funnel (XGBoost)", height=280))
    else:
    ...
    if not churn_df.empty and "loyalty_tier" in churn_df.columns and "churn_probability" in churn_df.columns:
        ...
        # ✅ FIX 2: L() helper
        fig_cl.update_layout(**L(xaxis=AXIS, yaxis=AXIS,
    ...
    return loading(html.Div([
        ...
        # ✅ FIX 1: kpi() function called — not kpi_ctx dict
        html.Div(style={"display":"flex","gap":"10px","marginBottom":"14px","flexWrap":"wrap"}, children=[
...
def _get_ai_answer(question: str, bk, chat_history: list = None) -> str:
    ...
    kpi_d = bk.get("kpi", [{}])[0] if isinstance(bk, dict) and bk.get("kpi") else {}
    total_records = int(kpi_d.get("total_bookings", 0))
...
KPIs: {total_records:,} bookings | {kpi_d.get('total_customers',0):,} customers | ₹{kpi_d.get('total_revenue',0):,.0f} revenue.
...
def generate_auto_insights(forecast_data, fraud_data, churn_data, val_data, failed_data, sla_data):
    ...
    if not sla_df.empty and "status" in sla_df.columns:
        ...
        try:
            ...
            insights_context["total_pipeline_runs"] = 0
